...
const SaveButton = (props) => {
  const { isSaved, onClickSave } = props;
  const iconColor = isSaved ? '#FF5A5F' : '#484848';
  const saveText = isSaved ? 'Saved' : 'Save';
  const fillOpacity = isSaved ? '1' : '0';
  return (
    <div className={css['save-button']}>
      <button type="button" onClick={onClickSave}>
        <div>
          <svg className={`${css['save-icon']} ${css['vertically-center']}`} viewBox="0 0 24 24" fill={iconColor} fillOpacity={fillOpacity} stroke={iconColor} strokeWidth="1" role="img" strokeLinecap="round" strokeLinejoin="round">
          ...
          <span className={css['vertically-center']}>{saveText}</span>
        </div>
      </button>
    </div>
  );
